...
def recognize_faces_in_directory(directory, batch_size=10, tolerance=0.6):
    ...
    for root, dirs, files in os.walk(directory):
        batch_images = []  # Armazenar as imagens atuais do lote
        batch_processed_images = []  # Armazenar os caminhos das imagens processadas no lote

        for idx, file_name in enumerate(files):
            if len(batch_images) >= batch_size:
                break  # Se o tamanho do lote for atingido, interrompa a iteração
                
            file_ext = os.path.splitext(file_name)[1].lower()
            if file_ext in filetypes:
                image_path = os.path.join(root, file_name)
                batch_images.append(image_path)
                batch_processed_images.append(image_path)
                ...
                if file_ext in ('.cr2', '.nef'):
                    ...
                    batch_images[-1] = jpg_path

        for image_path in batch_images:
            img = cv2.imread(image_path)
            rgb_img = cv2.cvtColor(img, cv2.COLOR_BGR2RGB)
            face_locations = face_recognition.face_locations(rgb_img)
            face_encodings = face_recognition.face_encodings(rgb_img, face_locations, 1)
